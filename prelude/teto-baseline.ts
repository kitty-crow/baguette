// Teto baseline intrinsic prelude selected by baguette.config.json.
...
@inline function compareExchangeI32(memoryHandle: usize, at: u32, expected: i32, replacement: i32): i32 {
  const old = load<i32>(at); if (old == expected) store<i32>(at, replacement); return old;
}
@inline function atomicAddI32(memoryHandle: usize, at: u32, value: i32): i32 {
  const old = load<i32>(at); store<i32>(at, old + value); return old;
}
...
@inline function atomicAddU64(memoryHandle: usize, at: u32, value: u64): u64 {
  const old = load<u64>(at); store<u64>(at, old + value); return old;
}
